...
public static class _9_1 {
    public static int Execute()
    {
        ...
        foreach (var line in lines)
        {
            ...
            sum += GetLastValue(list);
        }

        return sum;
    }

    public static int GetLastValue(List<int> range)
    {
        ...
        {
            var result = range.Last() + GetLastValue(newRange);
            return result;
        }
        else
        {
            var result = range.Last() + newRange.Last();
            return result;
        }
    }
}